...
int cert1(void)
   { 
    ...
    const char* caCert     = "./xmss-fal/certs/root.pem";
    const char* verifyCert = "./xmss-fal/certs/servercert.pem";
...
//to verify the signatures generated at root level in a cerificate chain
int process_verification(const char* cert_file, const char* key_file, const char* msg_file, const char* sig_file)
{
    ...
    if (ret == 0) {
        ret = wc_InitSphincsKey(&pub_key);
        check_ret("wc_sp_init", ret);
    }
...
//to import the public key that got extracted in der format and store it in pub_key
    if (ret == 0) {
        ...
        ret =  wc_ImportSphincsPublic(pub_der_buf, pub_der_len , &pub_key);
  ...
  //to read the signature from the given input file and use the public key extracted from certificate and verify the signature using the public key
    if (ret == 0) {
       ...
       long long unsigned int* outlen;
        ...
        read_buffer_from_file(sig_file, signature, sizeof(msg)+SPHINCS_SIG_SIZE);

       size_t signature_len = read_buffer_from_file(sig_file , signature, sizeof(msg)+SPHINCS_SIG_SIZE);
        if (signature_len == 0) {
        fprintf(stderr, "Failed to read signature from file.\n");
        return -1; // Handle the error as needed
        }
        ret = wc_SphincsVerify(msg, &outlen, signature, signature_len, &pub_key);
       check_ret("wc_sp_verify", ret);
    }

    printf("verify result: %s\n", ret == 0 ? "SUCCESS" : "FAILURE");

     wc_FreeSphincsKey(&priv_key);
     wc_FreeSphincsKey(&pub_key);
     wc_FreeRng(&rng);
    //wolfCrypt_Cleanup();
...
//to verify the signatures generated at ica and leaf level in a cerificate chain
int process_verification1(const char* cert_file, const char* key_file, const char* msg_file, const char* sig_file)
{
...
int main(int argc, char** argv) {
  /*  
    ...
    ret = process_verification1("icacert.pem", "icakey.pem", "f1.img.xz", "signature2.txt");
    if (ret != 0) return ret;

     // Process the third set of files
    ret = process_verification1("servercert.pem", "serverkey.pem", "f2.zip", "signature3.txt");
    if (ret != 0) return ret;
    end = clock();
    ...
    FILE *csv_file = fopen("sp1-fal1_secboot_ver.csv", "w");
    if (csv_file == NULL) {
