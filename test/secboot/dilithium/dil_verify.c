...
int process_verification(const char* cert_file, const char* key_file, const char* msg_file, const char* sig_file)
{
    ...
    //to read the signature from the given input file and use the public key extracted from certificate and verify the signature using the public key extractedd
    if (ret == 0) {
       ...
       /* make dummy msg */
        ...
        ret = wc_DilithiumVerify(msg, (size_t*)&outlen, signature, signature_len, &pub_key);
...
int main(int argc, char** argv) {
    ...
    FILE *csv_file = fopen("dil3_secboot_ver.csv", "w");
    if (csv_file == NULL) {
    ...
    for (int i = 1; i <= num_iterations; ++i) {
	 ...
	 // Process and record root-verification time
        start = clock();
        ...
        // Process and record ica-verification time
        start = clock();
        ...
        // Process and record server-verification time
        start = clock();
