   ...
   /* if (ret == 0) {
    ...
    if (ret == 0) {
       /* ret = wc_dilithium_sign_msg((const byte *)MESSAGE, sizeof(MESSAGE),
        ...
        ret=wc_XmssSign(signature, (long long unsigned int*)&signature_len, (const byte *)MESSAGE, msgLen, &priv_key);
    ...
    if (ret == 0) {
       ...
       /* make dummy msg */
        for (int i = 0; i < (int)sizeof(msg); i++)
           msg[i] = (byte)i;
       //msg
        printf("Message Contents before sig verify:\n");
           printf("msg Contents in ASCII:\n");
            for (int i = 0; i < (int)sizeof(msg); i++) {
       ...
       // Print the contents of the signature in hexadecimal format   bosch pqc
        ...
        printf("\n");
        //msg
        printf("Message Contents from sig:\n");
           for (int i = 0; i < (int)sizeof(msg); i++) {
                   if (msg[i] >= 32 && msg[i] <= 126) {
                          printf("%c", msg[i]);
                    } else {
