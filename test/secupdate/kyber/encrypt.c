...
int main() 
{
    ...
    FILE *csv_file = fopen("exp/kyber_enc.csv", "w");
    if (csv_file == NULL) {
    ...
    fprintf(csv_file,"%s" ,"encryption[ms]\n");
    
    // Loop to read from keys/kyber_public_key_1.txt to keys/kyber_public_key_1000.txt
    // and store the outputs in sharedkeys/kyber_shared_secret_1.txt to sharedkeys/kyber_shared_secret_1000.txt
    ...
    for (int i = 1; i <= 1000; ++i) {
        ...
        sprintf(public_key_filename, "keys/kyber_public_key_%d.txt", i);
        sprintf(shared_secret_filename, "sharedkeys/kyber_shared_secret_%d.txt", i);
        sprintf(cipher_text_filename, "ciphertexts/kyber_cipher_text_%d.txt", i);

        // Read the public key from file
        ret = readByteArrayFromFile(key.pub, KYBER_PUBLICKEYBYTES, public_key_filename);
        if (ret != 0) {
        ...
        if (ret != 0) {
            printf("Failed to encrypt with Kyber for key pair %d.\n", i);
            continue;
        }
        ...
        // Save the cipher text to file
        writeByteArrayToFile(key.ct, KYBER_CIPHERTEXTBYTES, cipher_text_filename);
