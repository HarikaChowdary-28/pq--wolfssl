...
int process_verification(const char* cert_file, const char* key_file, const char* msg_file, const char* sig_file)
{
    ...
    if (ret == 0) {
       ...
       /* make dummy msg */
        ...
        ret = wc_DilithiumVerify(msg, (long long unsigned int *)&outlen, signature, signature_len, &pub_key);
...
int main(int argc, char** argv) {
    ...
    FILE *csv_file = fopen("exp/dil2_sig_ver.csv", "w");
    if (csv_file == NULL) {
    ...
    for (int i = 1; i <= 1000; ++i) {
     ...
     // Construct file paths for each iteration
        sprintf(server_cert_path, "dilithium2/it%d/certs/servercert.pem", i);
        sprintf(server_key_path, "dilithium2/it%d/certs/serverkey.pem", i);
