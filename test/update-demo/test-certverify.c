...
#define CSV_FILENAME "dil2_certverify.csv"
...
int main()
{
    ...
    for (int i = 0; i < NUM_CERTS; i++) {
        char filename[50];
        snprintf(filename, sizeof(filename), "dilithium/it%d/rootcert.pem", i + 1);
        rootCertFiles[i] = strdup(filename);
        snprintf(filename, sizeof(filename), "dilithium/it%d/icacert.pem", i + 1);
        icaCertFiles[i] = strdup(filename);
        snprintf(filename, sizeof(filename), "dilithium/it%d/servercert.pem", i + 1);
    ...
    fprintf(csvFile, "RootToIcaTimeInSeconds, IcaToServerTimeInSeconds, TotalTimeInSeconds\n");
    ...
    for (int i = 0; i < NUM_CERTS; i++) {
        ...
        rootToIcaTime[i] = ((double)(endTime - startTime)) / CLOCKS_PER_SEC;
        ...
        endTime = clock();
        // Calculate elapsed time
        icaToServerTime[i] = ((double)(endTime - startTime)) / CLOCKS_PER_SEC;
